...
def hpm(train: np.ndarray, test: np.ndarray, parameters: dict) -> np.ndarray:
    ...
    parameters : dict
        ...
        - segmenter_params (dict): Parameters for the segmenter.
    ...
    segmenter_params = parameters.get("segmenter_params") or {
        'window_size': 200,
    }
    segmenter = segmenter(params=segmenter_params)

    # Set up standard HPM StructuralProminenceNode parameters if missing:
    node_params = parameters.get("node_params") or {}
    node_params["structural_threshold"] = (
        node_params.get("structural_threshold")
        or (lambda depth: 0.5)
    )
    node_params["prominence_threshold"] = (
        node_params.get("prominence_threshold")
        or (lambda depth: 0.5)
    )

    # Set up standard HPM discretiser parameters if missing:
    discretiser_params = parameters.get("discretiser_params") or {}
    discretiser_params["max_depth"] = (
        discretiser_params.get("max_depth")
        or (lambda d: int(np.floor(np.log(len(d)))))
    )
    discretiser_params["frames"] = (
        discretiser_params.get("frames")
        or (lambda depth: 2 ** depth)
    )
    discretiser_params["node_type"] = (
        discretiser_params.get("node_type")
        or StructuralProminenceNode
    )
    ...
    training_chains = [
        discretiser.discretise(s) for s in segmenter.segment(train)
    ]
    testing_chains = [
        discretiser.discretise(s) for s in segmenter.segment(test)
    ]
    ...
    if context_size is not None:
        for i in range(0, len(testing_chains) - context_size + 1):
            ...
            end_point = min(
                (i + context_size - 1) * segmenter.step_size
                + segmenter.window_size,
                len(test),
            )
